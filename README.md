...
2. **Photo archiving** — when the `library/` folder exceeds a configurable size limit (in GB), moves the oldest photos to external storage while updating the Immich database, without any metadata loss.
...
triggers when the library exceeds `ARCHIVE_LIBRARY_MAX_GB` and moves the oldest photos
until the library drops to `ARCHIVE_LIBRARY_TARGET_GB` (1 GB = 1024³ bytes). The `setup`
wizard sets both boundaries: enter them as GB (e.g. `200`) or as a percentage of the host
disk (e.g. `80%`, converted to GB), and it shows the current library size and the disk's
total/free space as hints.
